...
렌더 서비스가 도는 VPS(`155.94.154.102`, 도메인 `cardnews.imgmap.shop`)에 Caddy·Docker·프로세스 관리 같은 작업이 필요하면 **사용자에게 대리 실행을 요청하지 말고 직접 수행한다.**

```bash
ssh root@155.94.154.102 "<명령어>"
```

- 진단(`ss -ltnp` · `journalctl -u caddy` · `systemctl status` · 설정 파일 읽기)과 기동·리로드는 바로 실행한다
- 설정 파일을 고칠 때는 **고치기 전 원본을 먼저 읽어 확인**하고, 덮어쓰기 전에 백업을 남긴다
- 되돌리기 어려운 작업은 실행 전에 사용자에게 알린다: 디스크·데이터 삭제, 방화벽 규칙 축소(SSH 잠김 위험), 시스템 패키지 대량 제거, 인증서·키 폐기
- 서버에서 얻은 출력에 토큰·키가 섞여 있으면 대화창에 그대로 옮기지 않는다

렌더 서비스 기동 규약은 [`services/render/README.md`](services/render/README.md)를 따른다. 포트는 Caddy가 넘기는 값과 맞춰야 한다(`PORT`, 기본 4000).

## 8. 현재 상태

- **Phase 0** (상표·도메인 검증) 미완 — 브랜드명은 i18n 키로만 노출해 교체 비용을 낮춘다
- **화면·기능은 전 구간 구축 완료** — 생성·Board·Deck·블로그·캘린더·성과·계정 연동·자동 DM·댓글 인박스·예약 발행·릴스 영상
- 남은 것은 기능이 아니라 **실동작 검증**이다. 상세와 리스크는 [`docs/project_status.md`](docs/project_status.md) §4·§6이 단일 진실원천이며, **작업 시작 전 그 문서를 먼저 읽는다**
