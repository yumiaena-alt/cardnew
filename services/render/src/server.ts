 ...
 *
 * 의존성을 최소화하려고 Node 기본 http를 쓴다. 엔드포인트가 3개뿐이라
 * 프레임워크를 얹을 이유가 없다.
 */
...
async function readJsonBody(req: IncomingMessage): Promise<unknown> {
  ...
  for await (const chunk of req) {
    ...
    if (total > MAX_BODY_BYTES) {
      throw new HttpError(413, `요청 본문이 너무 큽니다 (최대 ${MAX_BODY_BYTES} bytes)`);
    }
    chunks.push(buf);
  }
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/health') {
      sendJson(res, 200, { ok: true, browserRunning: pool.isRunning });
      return;
    }
